...
@export_range(0.0, 1.0, 0.01) var clouds_coverage: float = 0.0:
	set(value):
...
@export_range(0.0, 1.0, 0.01) var clouds_height: float = 1.0:
	set(value):
		clouds_height = value;
		if is_inside_tree():
...
@export_range(0.0, 1.0, 0.01) var wind_strength: float = 0.04:
	set(value):
		wind_strength = value;
		if is_inside_tree():
...
@export var wind_direction: Vector2 = Vector2(1.0,0.0):
	set(value):
		...
		if is_inside_tree():
			sky_shader.set_shader_parameter("WIND",Vector3 (wind_direction.x,0.0,wind_direction.y)*wind_strength);
@export_range(0.0, 0.5) var moon_size: float = 0.03:
	set(value):
		moon_size = value;
		if is_inside_tree():
...
@export var moon_tint: Color = Color(1, 0.834, 0.612):
	set(value):
		moon_tint = value;
		if is_inside_tree():
...
func set_time():
	...
	var light_energy: float = smoothstep(sunset_offset, 0.4, sun_pos.y) * 1.3;# light intensity depending on the height of the sun

	light_energy = clamp(light_energy, night_level_light, 2.0);
	if time_of_day > (1.0/24.0*7.0) && time_of_day < (1.0/24.0*8.0) :
		light_energy *= 5.0;
		
	light_energy = min(0.8, light_energy)
	...
	if sun_height < sunset_range:
		...
		light_color=sunset_light #lerp(sunset_light, day_light, clamp((sun_height-sunset_range)/sunset_range,0.0,1.0))
	...
	environment.fog_light_color = (light_color * 0.5 if (sun_pos.y < 0.0) else light_color) ;
	environment.fog_light_color.a = 1.0;
...
func _process(delta:float):
	...
	##sun_moon.light_energy = lighting_strength;
...
func get_normalized_hour() -> float:
	...
	var hour = time_dico.hour / 24.0
	var min = time_dico.minute / 60.0 / 24.0
	return hour + min
